...
epoch=$(python3 scripts/source-date-epoch.py "${epoch_arguments[@]}")
...
package_cpack() {
    ...
    SOURCE_DATE_EPOCH="$epoch" \
        ./scripts/normalize-cpack-packages.sh "$release"
...
for pass in first second; do
    for artifact in "$deb" "$tgz" "$arch"; do
        python3 scripts/verify-reproducible-archive.py \
