...
GENERATOR = ROOT / "scripts" / "generate-release-manifest.py"
VERIFIER = ROOT / "scripts" / "verify-release-manifest.py"
IDENTITY_GENERATOR = ROOT / "scripts" / "generate-source-identity.py"
PIN_VERIFIER = ROOT / "scripts" / "verify-pinned-checkouts.py"
SIGNER = ROOT / "scripts" / "sign-release-manifest.sh"
SIGNATURE_VERIFIER = ROOT / "scripts" / "verify-release-signature.sh"
...
class ReleaseIntegrityTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        parity = json.loads(
            (ROOT / "protocol/desktop-client-parity.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    def build_artifacts(self, identity: Path) -> None:
        ...
        self.run_command(
            [
                "bash",
                str(ROOT / "scripts/write-arch-package.sh"),
    ...
    def test_stale_artifacts_after_runtime_change_are_rejected(self) -> None:
        ...
        dependencies["kodosi"]["commit"] = self.head(self.runtime)
        self.write_json(self.dependencies, dependencies)
        parity = json.loads(self.parity.read_text(encoding="utf-8"))
        parity["baseline"]["runtimeCommit"] = self.head(self.runtime)
        self.write_json(self.parity, parity)
        self.generate(expect_success=False)
    ...
    def test_gpg_contract_is_offline_exact_and_rejects_bad_statuses(self) -> None:
        sign = SIGNER.read_text(encoding="utf-8")
        verify = SIGNATURE_VERIFIER.read_text(encoding="utf-8")
        self.assertIn("--armor --detach-sign", sign)
        self.assertIn("--no-auto-key-retrieve", sign)
        self.assertIn('--local-user "$fingerprint!"', sign)
        self.assertIn("--no-auto-key-retrieve", verify)
        self.assertIn("--structure-only", verify)
        self.assertNotIn("KODOSI_SOURCE_ROOT", verify)
        self.assertIn("VALIDSIG", verify)
        for status in (
            "EXPKEYSIG",
            "REVKEYSIG",
            "KEYEXPIRED",
        ...
        self.assertNotIn("--generate-key", sign)
        self.assertNotIn("--import", sign)
        self.assertNotIn("--export", sign)
